...
双轨引用回复中继：
群组协同：任意授权群组内的管理员长按/右键点击访客原消息或下方信息资料卡并选择“回复（Reply）”，机器人即可自动将回复内容送达该访客私聊。
...
点击 Create Namespace，命名为 `tg-bot-db`（名称可自定义）。
---
步骤二：创建 Worker 并绑定 KV 数据库
导航至 Workers & Pages -> 点击 Create Application -> Create Worker。
输入服务名称（如 `tg-contact-bot`），点击 Deploy。
进入该 Worker 的设置界面，点击 Settings -> Bindings（或 Variables and Secrets）。
在 KV Namespace Bindings 区域点击 Add：
...
---
步骤四：部署 Worker 业务代码
在 Worker 管理页面点击 Edit code 进入代码编辑器。
清空默认生成的脚本，将项目中的完整 `worker.js` 代码粘贴进去。
点击右上角 Deploy 进行发布。
---
...
---
运行与自检验证
健康自检：在浏览器直接访问你的 Webhook 地址（例如 `https://tgchat.2pac.pp.ua/xiagefei120`），若配置正确会返回纯文本健康报告：
```text
   Worker 多群组版运行正常！
   Token 状态: 已配置 ✅
   KV 状态: 已成功绑定 ✅
   ...
   ```
群组管理员赋权：务必将机器人拉入 `OWNER_GROUP_IDS` 中配置的所有群组，并将机器人提升为群管理员（至少开启“发送消息”和“删除消息”权限）。
唤出管理面板：
...
KV 命名空间绑定名称是否严格大写为 `BOT_DB`；
环境变量 `BOT_TOKEN` 是否已正确填入。
Q3: 管理群内收不到访客消息推送？
A: 请排查以下项：
群组数字 ID 是否以 `-100` 开头；
机器人是否已被拉进群组，且是否赋予了发消息的管理员权限；
访客消息是否触发了黑名单关键词或链接引流规则被系统直接拦截。
